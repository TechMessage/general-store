...
; (function (window, undefined) {
	...
	function YnTree(ele, options) {
		...
		if (!ele || !ele.nodeName || (ele.nodeType != 1)) {
			throw "YnTree 第一个参数必须是一个dom元素，容器！";
		}
		// 当前Yntree实例的属性 ele是当前的容器dom ,外部参数
		this.ele = ele;
		...
		this.options = options; //外部参数，构造实例时传入的参数对象

		!this.options.data ? this.options.data = [] : ""; // options对象中必须有data属性，且为数组，该属性就是存储树形渲染数据

		// 如果数据不是一个数组，则将其转换成数组，数组是单个对象时转为数组
		if (YnTree.getType(this.options.data) == "object") {
			this.options.data = [this.options.data];
		}
		// 存储节点数据，存储的是整个树形结构的所有节点
		this.data = [];
		
		// 存储平行的节点数据，方便后期查找父元素， 存储的是每个单独的节点，类似1维数组，每个节点都有父子关系的pid 关联
		this.parallel = [];

		//当前ynTree实例的id，唯一标识， 当前页面可以初始化多个树形实例，每个实例给一个id表示，方便追踪
		this.id = "yn_tree" + (++count); 
		
		// 存储最外层的树结构
		this.tree = YnTree.createDomByString('<ul class="yn-tree" id="' + this.id + '"></ul>'); //最外成的结构
	...
	YnTree.prototype._init = function () {
		...
		if (this.options.data.length > 0) {
			...
			// 根据外部传入的options中data数据，遍历，然后每个节点数据生成对象，拷贝this.data中
			this._copyData(this.options.data, this.data); 

			// 根据数据this.data生成dom元素
	...
	YnTree.prototype._copyData = function (data, parent) {
		// 缓存this，这个this指向的是yntree实例
		var that = this;

		data = data || that.options.data; //兜底, data是原始option中提供的数据
		// 判断类型，非数组要包装一下
		YnTree.getType(data) != "array" ? (data = [data]) : "";

		// 遍历数组数据，forEach是自己封装的，静态方法
		YnTree.forEach(data, function (index, item) {
			...
			if (item.children) {
				...
				//递归，把item.children 拷贝到obj.children中
	...
	YnTree.prototype._createDom = function (data, parent) {
		...
		YnTree.forEach(data, function (index, item) {
			var html = '',
				id = 'yn_tree_input' + (++inputCount), //生成唯一标识id
			...
			// spread 这个类名是展开的，默认父节点全部展开
			// parent 是父节点的id
			html += '<li class="yn-tree-li spread" id="' + id + '_li" ' + (parent ? ('pid="' + parent.id + '"') : "") + '>';
			...
			// 给data中每个数据节点添加 三个属性id, pid, dom
			item.id = id;
			item.pid = (parent ? parent.id : null);
			item.dom = YnTree.createDomByString(html);//将生成的dom对象复制给自己属性dom
	...
	YnTree.on = function (ele, type, fn) {
		if (document.addEventListener) {
			ele.addEventListener(type, fn, false); //false 是选择事件的冒泡阶段
	...
	YnTree.extend = function (target, obj) {
		// for(var .. in ) 会遍历对象上所有的属性，包括原型上的，
		// hasOwnProperty方法可以过滤掉非对象自身的属性，过滤调原型上的属性和方法
	 ...
	 * 
	 */
	function CompositeLeaf(node, type, ynTreeId) {
		this.type = type || "";

		// 浅拷贝，把原数据中的属性都拷贝新的对象上，如果有children属性的话，把内容清空
		// 因为原数据中父节点是有children属性的，是一个数组

		YnTree.extend(this, node || {});
